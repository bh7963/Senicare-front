...
import { ACCESS_TOKEN, CS_APSOLUTE_PATH } from 'src/constants';
import { access } from 'fs';
import { fileUploadeRequest, getNurseListRequest, postCustomerReqeust } from 'src/apis';
...
export default function CSWrite() {
    ...
    const onPostClickHandler = async () => {
        ...
        postCustomerReqeust(requestBody, accessToken).then(postCustomerResponse);
    };

    // effect: modal 첫 로드 시 요양사 리스트 불러오기 함수 //
    useEffect(()=>{
        const accessToken = cookies[ACCESS_TOKEN];
        if (!accessToken) return;
